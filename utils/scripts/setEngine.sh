...
echo "*--"
wget -q http://apache.mindstudios.com/maven/maven-3/3.3.9/binaries/apache-maven-3.3.9-bin.zip
if [ -f "apache-maven-3.3.9-bin.zip" ]; then
...
if [ -f "apache-karaf-3.0.5.tar.gz" ]; then
...
if [ -d "ryu" ]; then
	...
	echo 'export PYTHONPATH=/usr/local/lib/python2.7/dist-packages/ryu' >> ~/.bashrc
	cd ..
fi
