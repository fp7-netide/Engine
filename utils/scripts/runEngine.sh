...
if [ -z "$1" ] || [ "$1" == "-h" ] || [ "$1" == "--help" ]
then 
	echo "Usage: <runEngine.sh> <-b (backend) / -c (core) / -s (shim) / -m (Mininet)> <type (Ryu/Floodligh; Java/Python; Ryu/ODL/ONOS; netide-topo)>"
	exit 0
fi
...
then 
	...
	then
		...
		echo "Error! Backend" $2 "not available... <choose type (Ryu/Floodligh)>"
	fi
...
then 
	...
	then
		...
		echo "karaf> feature:repo-add mvn:eu.netide.core/core.features/1.0.0.0-SNAPSHOT/xml/features (1.1.0) (1.0.1.0)"
		echo "karaf> feature:install netide-core (core)"
		echo "karaf> netide:loadcomposition /path/to/composition"
