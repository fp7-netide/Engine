...
class Firewall(app_manager.RyuApp):
    ...
    def add_flow(self, datapath, match, actions, idle_to, hard_to, priority):
    ...
    def Configure_stateful_FW(self, msg):
        ...
        # Forward packets from inside to outside and also install the reverse rule with idle_to=8 sec
            ...
            self.add_flow(datapath, match, actions, 8, 0, 10)
            ...
            # forward the packet
            self.forwardPacket(msg, 1)
	    print(">>> FW: FORWARD PACKET from %s to port 1" % (msg.in_port)) 

        elif msg.in_port == 1:
            match = datapath.ofproto_parser.OFPMatch(in_port=FW_OUTPORT, dl_type = ETH_IP, dl_src=haddr_to_bin(hwsrc), dl_dst=haddr_to_bin(hwdst))
            actions = []
            self.add_flow(datapath, match, actions, 8, 0, 5)
