 ...
 */
package eu.netide.core.globalfib.flow;
...
import static eu.netide.core.globalfib.flow.OpenFlowValueMapper.lookupChannelSpacing;
import static eu.netide.core.globalfib.flow.OpenFlowValueMapper.lookupGridType;
import static eu.netide.core.globalfib.flow.OpenFlowValueMapper.lookupOchSignalType;
import static eu.netide.core.globalfib.flow.OpenFlowValueMapper.lookupOduSignalType;
        ...
        case EXP_OCH_SIG_ID:
            try {
                ...
                builder.add(modL0Lambda(Lambda.ochSignal(
                        OpenFlowValueMapper.lookupGridType(circuitSignalID.getGridType()),
                        OpenFlowValueMapper.lookupChannelSpacing(circuitSignalID.getChannelSpacing()),
        ...
        for (MatchField<?> field : match.getMatchFields()) {
            ...
            case OCH_SIGID:
                ...
                builder.add(matchLambda(Lambda.ochSignal(
                                OpenFlowValueMapper.lookupGridType(sigId.getGridType()), OpenFlowValueMapper.lookupChannelSpacing(sigId.getChannelSpacing()),
                                sigId.getChannelNumber(), sigId.getSpectralWidth())
                ));
                break;
            case OCH_SIGTYPE:
                U8 sigType = match.get(MatchField.OCH_SIGTYPE);
                builder.add(matchOchSignalType(OpenFlowValueMapper.lookupOchSignalType((byte) sigType.getValue())));
                break;
            case EXP_OCH_SIG_ID:
                try {
                    CircuitSignalID expSigId = match.get(MatchField.EXP_OCH_SIG_ID);
                    builder.add(matchLambda(Lambda.ochSignal(
                            OpenFlowValueMapper.lookupGridType(expSigId.getGridType()), OpenFlowValueMapper.lookupChannelSpacing(expSigId.getChannelSpacing()),
            ...
            case EXP_OCH_SIGTYPE:
                try {
                    ...
                    builder.add(matchOchSignalType(OpenFlowValueMapper.lookupOchSignalType((byte) expOchSigType.getValue())));
                } catch (NoMappingFoundException e) {
                    log.warn(e.getMessage());
                    break;
                }
            ...
            case EXP_ODU_SIGTYPE:
                try {
                    ...
                    builder.add(matchOduSignalType(OpenFlowValueMapper.lookupOduSignalType((byte) oduSigType.getValue())));
                } catch (NoMappingFoundException e) {
                    log.warn(e.getMessage());
                    break;
                }
